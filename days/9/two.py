...
def main():
    ...
    for low in low_points:
        counted = set()
        basin_size = get_basin_size(counted, height_map, low[0], low[1])
        basins_sizes.append(basin_size)
    
    print(reduce(operator.mul, sorted(basins_sizes)[-3:]))
    

def get_basin_size(counted, height_map, i, j):
    val = height_map[i][j]
    if val == 9 or tuple([i,j]) in counted:
        return 0

    valid_dirs = get_valid_dirs(height_map, i, j)
    counted.add(tuple([i,j]))
    
    size = 1
    for d in valid_dirs:
        if d == 'l' and val < height_map[i][j-1]:
            size += get_basin_size(counted, height_map, i, j-1)
        if d == 'r' and val < height_map[i][j+1]:
            size += get_basin_size(counted, height_map, i, j+1)
        if d == 'u' and val < height_map[i-1][j]:
            size += get_basin_size(counted, height_map, i-1, j)
