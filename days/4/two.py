...
def main():
    ...
    for draw in draws:
        for board_idx in range(len(boards)):
            if not boards[board_idx]:
                continue
            for row_idx in range(len(boards[board_idx])):
                for item_idx in range(len(boards[board_idx][row_idx])):
                    if boards[board_idx][row_idx][item_idx] == draw:
                        boards[board_idx][row_idx][item_idx] = None
            if has_board_won(boards[board_idx]):
                winning_board = boards[board_idx]
                winning_draw = draw
                boards[board_idx] = None

        remaining_boards = list(filter(lambda x: x != None, boards))
